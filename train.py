...
if fair_sampling:
    ...
    from evaluation import evalrank, i2t, t2i, AverageMeter, LogCollector, encode_data, shard_xattn_t2i, shard_xattn_i2t
...
def main():
    ...
    parser.add_argument('--test_data', action='store_true',
                        help='testing the model on test set.')
    parser.add_argument('--neutralize', action='store_true',
                        help='whether neutralize the text for test data.')
    ...
    if opt.test_data:
        ...
        if opt.resume:
            if os.path.isfile(opt.resume):
                evalrank(opt.resume, None, 'test', False, opt)
                '''
                test_loader = data.get_test_loader('test', opt.data_name, vocab, opt.batch_size, opt.workers, opt)
