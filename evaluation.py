...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    max_n_word = 0
    for i, (images, captions, lengths, ids, gender) in enumerate(data_loader):
    ...
    for i, (images, captions, lengths, ids, gender) in enumerate(data_loader):
        ...
        if img_embs is None:
            if img_emb.dim() == 3:
                img_embs = np.zeros((int(len(data_loader.dataset)/5), img_emb.size(1), img_emb.size(2)))
            else:
                img_embs = np.zeros((int(len(data_loader.dataset)/5), img_emb.size(1)))
            cap_embs = np.zeros((len(data_loader.dataset), max_n_word, cap_emb.size(2)))
            cap_lens = [0] * len(data_loader.dataset)
            gender_list = [0] * len(data_loader.dataset)
        # cache embeddings
        #print(ids)
        #print(img_embs.shape)
        #img_embs[ids] = img_emb.data.cpu().numpy().copy()
        img_embs_temp = img_emb.data.cpu().numpy().copy()
        #print(img_embs_temp.shape)
        cap_embs[ids,:max(lengths),:] = cap_emb.data.cpu().numpy().copy()
        for j, nid in enumerate(ids):
            if nid%5 == 0:
                new_nid = int(nid/5)
                img_embs[new_nid] = img_embs_temp[j,:,:]
            cap_lens[nid] = cap_len[j]
            gender_list[nid] = gender[j]
        '''
        ...
        '''
        del images, captions
    return img_embs, cap_embs, cap_lens, gender_list


def evalrank(model_path, data_path=None, split='dev', fold5=False, opt=None):
    """
    ...
    img_embs, cap_embs, cap_lens, gender_lists = encode_data(model, data_loader)
    print('Images: %d, Captions: %d' %
          (img_embs.shape[0] / 5, cap_embs.shape[0]))
    
    genders = []
    for i in range(0, len(gender_lists), 5):
        gender_counter = Counter(gender_lists[i:i+5])
        g = gender_counter.most_common(1)[0][0]
        '''
        gender_counter = gender_lists[i:i+5]
        if 3 in gender_counter:
            g = 0
        elif 1 in gender_counter and 2 in gender_counter:
            g = 0
        elif 1 in gender_counter and 2 not in gender_counter:
            g = 1
        elif 1 not in gender_counter and 2 in gender_counter:
            g = 2    
        else:
            g = 0  
        '''    
        #genders.append(g)
        print(g, "|||", gender_lists[i +0], gender_lists[i +1] , gender_lists[i +2] , gender_lists[i +3] , gender_lists[i +4])
    
    #genders = [gender_lists[i] for i in range(0, len(gender_lists), 5)]
    ...
    if not fold5:
        ...
        (r1,r5,r10), biases = evaluate_bias(sims, np.array(genders), return_ranks=True)
        ...
        print("Text to image: %.1f %.1f %.1f" % (r1,r5,r10))
        print("Gender bias:", biases)
    else:
        ...
        for i in range(5):
            ...
            genders = gender_lists[i * 5000:(i + 1) * 5000:5]
...
def evaluate_bias(similarity: np.ndarray, gender, return_ranks=False):
    """
    Evaluate the recall and bias performance
    Input:
        similarity: A numpy array of shape [N, 5N].
    Output:
        recall: Recall@1, Recall@5, Recall@10
        bias:   Bias@[1...10]
    """
    npt = similarity.shape[0]
    assert npt == len(gender)

    ranks = np.zeros(5*npt)
    top1 = np.zeros(5 * npt)
    male = np.zeros(5*npt)
    female = np.zeros(5*npt)
    ...
    for index in range(npt):
        for i in range(5):
            inds = np.argsort(similarity[5 * index + i])[::-1]
    ...
    for k in range(1, 11):
        ...
        for index in range(npt):
            for i in range(5):
                inds = np.argsort(similarity[5 * index + i])[::-1]
                inds = inds[:k]
                male[5 * index + i] = (gender[inds] == 1).sum()
                female[5 * index + i] = (gender[inds] == 2).sum()
...
if __name__ == "__main__":
    sim = np.random.rand(1000, 5000)
    gender = np.random.rand(1000,)
    evaluate_bias(sim, gender)