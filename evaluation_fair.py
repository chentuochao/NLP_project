...
import torch
from fairsample import SCAN, xattn_score_t2i, xattn_score_i2t
...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    for i, (images, captions, lengths, ids, gender) in enumerate(data_loader):
        ...
        if img_embs is None:
            ...
            cap_attrs = [0] * len(data_loader.dataset)
        # cache embeddings
        img_embs[ids] = img_emb.data.cpu().numpy().copy()
        cap_embs[ids,:max(lengths),:] = cap_emb.data.cpu().numpy().copy()
        for j, nid in enumerate(ids):
            cap_lens[nid] = cap_len[j]
            cap_attrs[nid] = cap_attr[j]
...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    img_embs, cap_embs, cap_lens, cap_attrs = encode_data(model, data_loader)
